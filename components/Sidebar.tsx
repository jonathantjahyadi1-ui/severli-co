    ...
    <>
      ...
      {!isOpen && (
        <button
          type="button"
          onClick={onOpen}
          className="fixed left-5 top-5 z-50 flex h-11 w-11
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 z-40 flex h-screen w-64
                    flex-col overflow-y-auto border-r border-gray-200
                    bg-white px-5 py-8 shadow-xl transition-transform
      ...
      >
        ...
        <div className="relative mb-10 flex h-[70px] items-center justify-center">
          ...
          <button
            type="button"
            onClick={onClose}
            className="absolute -right-1 -top-3 flex h-9 w-9
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 rounded-lg px-3.5
                            py-2.5 text-[13px] font-medium tracking-wide
